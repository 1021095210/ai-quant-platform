...
class StaticAssetTests(unittest.TestCase):
    ...
    def test_replay_assets_include_screenshot_ocr_flow(self) -> None:
        ...
        self.assertIn('id="trade-parse-filter-kind"', replay_html)
        self.assertIn('id="trade-parse-filter-status"', replay_html)
        self.assertIn('multiple', replay_html)
        ...
        self.assertIn("/retry", replay_js)
        self.assertIn("task-progress-bar", replay_js)
    ...
    def test_strategy_and_backtests_assets_include_capability_guidance(self) -> None:
        ...
        self.assertIn('id="strategy-task-center"', strategy_html)
        self.assertIn('id="strategy-refresh-tasks-btn"', strategy_html)
        self.assertIn("/api/v1/platform/capabilities", strategy_js)
        self.assertIn("/api/v1/strategies/generations", strategy_js)
        self.assertIn("strategy-open-task-btn", strategy_js)
    ...
    def test_mentor_and_assistant_assets_include_llm_profile_selectors(self) -> None:
        ...
        self.assertIn('id="mentor-task-center"', mentor_html)
        self.assertIn('id="mentor-refresh-tasks-btn"', mentor_html)
        self.assertIn("fetchLlmProfiles", mentor_js)
        self.assertIn("/api/v1/mentor/ask-tasks", mentor_js)
        self.assertIn("mentor-open-task-btn", mentor_js)
        self.assertIn('id="assistant-llm-profile"', assistant_html)
        self.assertIn('id="assistant-task-center" class="list empty-state bounded-scroll bounded-scroll-lg"', assistant_html)
        ...
        self.assertIn("current_position_evidence", assistant_js)
        self.assertIn("止跌/企稳标准", assistant_js)
